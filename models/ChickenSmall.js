...
export default class ChickenSmall extends Chicken {
  ...
  startJump(currentTime) {
    ...
    console.log(`🐔 [ChickenSmall] Jump started! Next jump in ${this.nextJumpDelay.toFixed(1)}s`);
  }
  ...
  /** Override draw method to show jump debug info if needed */
  draw(ctx) {
    ...
    // Debug info for jumping
    if (this.debug && this.isJumping) {
      ctx.fillStyle = 'yellow';
      ctx.font = '10px Arial';
      ctx.fillText('JUMP', this.x, this.y - 5);
      
      // Jump trajectory indicator
      ctx.strokeStyle = 'yellow';
      ctx.beginPath();
      ctx.moveTo(this.x + this.width/2, this.y + this.height);
      ctx.lineTo(this.x + this.width/2, this.groundY + this.height);
      ctx.stroke();
    }
  }

  /** Get current chicken state info for debugging */
  getChickenInfo() {
    return {
      position: { x: Math.round(this.x), y: Math.round(this.y) },
      groundY: this.groundY,
      isJumping: this.isJumping,
      speedY: Math.round(this.speedY * 100) / 100,
      nextJumpIn: Math.max(0, this.nextJumpDelay - (performance.now() - this.lastJumpTime) / 1000),
      isDead: this.isDead,
