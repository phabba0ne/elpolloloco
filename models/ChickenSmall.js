...
export default class ChickenSmall extends Chicken {
  ...
  /**
   * Check if jump should start
   */
  shouldJump(currentTime) {
    if (this.isJumping || this.isDead) return false;
    const timeSinceLast = (currentTime - this.lastJumpTime) / 1000;
    return timeSinceLast >= this.nextJumpDelay && Math.random() < this.jumpChance;
  }
  ...
  die() {
    ...
    AudioHub.playOne("CHICKENSMALL_SOUNDS", "dead");
  }
  ...
  updateJumping(deltaTime, currentTime) {
    ...
    this.y = this.groundY;
    this.speedY = 0;
    if (currentTime - this.jumpStartTime >= this.minJumpDuration) {
      this.isJumping = false;
    }
  }
  ...
  update(deltaTime, character) {
    ...
    if (!this.isDead) {
      ...
      if (this.shouldJump(currentTime)) this.startJump(currentTime);
   ...
   */
  draw(ctx) {
    if (typeof super.draw === "function") return super.draw(ctx);
  ...
  /**
   * Check if chicken is on the ground
   */
  isOnGround() {
